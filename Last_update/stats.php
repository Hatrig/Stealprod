...
// Конец счетчика
	...
	// логов с самп-рп
	$srplogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Samp-Rp.Ru%'");

	// логов с Адванс-рп
	$arplogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Advance%'");

	// логов с Диамонд-рп
	$drplogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Diamond%'");

	// логов с Аризонас-рп
	$azrplogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Arizona%'");

	// логов с Evolve
	$evolvelogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Evolve-Rp.Ru%'");

	// логов с Grand
	$grandlogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Grand%'");

	// логов с Trinity
	$trinitylogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Trinity%'");

	// логов с Luxe
	$luxelogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Luxe%'");

	// логов с Revent rp
	$reventlogs = $db->getOne("SELECT COUNT(*) FROM logs WHERE id=$id AND server LIKE '%Luxe%'");

	$accountkol = $db->getOne("SELECT COUNT(DISTINCT login) as c FROM logs WHERE id=$id");


	$accountkol1lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 1'");
	$accountkol2lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 2'");
	$accountkol3lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 3'");
	$accountkol4lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 4'");
	$accountkol5lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 5'");
	$accountkol6lvl = $db->getOne("SELECT COUNT(DISTINCT login) FROM logs WHERE id=$id AND st LIKE '%lvl: 6'");

	$lvl1and3 = $accountkol1lvl+$accountkol2lvl + $accountkol3lvl;
	$lvl4and6 = $accountkol4lvl+$accountkol5lvl + $accountkol6lvl;
	$lvlmore6 = $accountkol - ($lvl1and3+$lvl4and6);
...
<head>
    ...
    <title>Статистика панели - Stealprod</title>
            ...
            <div class="row">
                ...
                <div class="main_lk col-md-9">
                    ...
                    <div class="stats_panel">
                        <p style="color: #fff">Общее число аккаунтов в логах: <span style="color: #62fdbd;"><?php echo $accountkol; ?></span><br>
                            Аккаунтов 1-3 лвл: <span style="color: #62fdbd;"><?php echo $lvl1and3; ?></span><br>
                            Аккаунтов 3-6: лвл: <span style="color: #62fdbd;"><?php echo $lvl4and6; ?></span><br>
                            Аккаунтов 6+ лвл (и не авторизованные): <span style="color: #62fdbd;"><?php echo $lvlmore6; ?></span><br>
                        </p></td></tr></table><br>
                        <style>
                            canvas {
                        ...
                        <div class="nahuycanvas" style="width: 700px; height: 400px;"><canvas id="chartProfit2"></canvas></div>
                        <script>
                            var ctx = document.getElementById("chartProfit2");
                            Chart.defaults.global.defaultFontColor = 'white';


                            var chartProfit = new Chart(ctx, {
                                type: 'doughnut',
                                data: {
                                    labels: [
                                        "SAMP-RP", "ADVANCE", "ARIZONA", "DIAMOND", "EVOLVE", "TRINITY", "GRAND", ],


                                    datasets: [{
                                        label: 'Приход логов',
                                        data: [
											<?php echo $srplogs ?>, <?php echo $arplogs ?>, <?php echo $azrplogs ?>, <?php echo $drplogs ?>, <?php echo $evolvelogs ?>, <?php echo $trinitylogs ?>, <?php echo $grandlogs ?> ],
                                        backgroundColor: [ 'rgba(255, 0, 102, 0.5)', 'rgba(0, 228, 255, 0.5)', 'rgba(0, 255, 144, 0.5)', 'rgba(108, 255, 0, 0.5)', 'rgba(255, 216, 0, 0.8)', 'rgba(192, 0, 255, 0.8)', 'rgba(251, 48, 38, 0.8)', ],
    ...
    <div class="modal-wrapper dl__steal">
        <div class="modal-content-wrapper">
            ...
            <div class="modal-content">
				...
				<? } if ($lua == 1) { ?>
                    <a class="a-button" href="/php/download.php?file=plugin&format=lua" target="_blank">Скачать LUA</a>
				<? } ?>
